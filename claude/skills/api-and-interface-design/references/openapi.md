    ...
    Problem:                          # RFC 7807 Problem Details
      ...
      properties:
        type:
          type: string
          format: uri
          description: Stable URI identifying the error class. Clients branch on this.
          example: https://api.example.com/errors/resource-not-found
        title:
          type: string
          description: Short summary, fixed per type.
        ...
        instance:
          type: string
          format: uri
          description: URI identifying this occurrence — usually a request ID.
          example: /requests/req-abc123
        errors:                       # extension — field-level validation failures
          type: array
          items:
            type: object
            required:
...
Define `Problem` once and `$ref` it from every error response. A spec that inlines error schemas per endpoint is how error formats drift apart.
