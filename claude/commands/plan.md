...
- **Agent** `planner` — Stage 1 executor for read-only codebase analysis: requirements, architecture review with file references, phased step breakdown, dependencies, and risk/complexity assessment. Read-only (Read/Grep/Glob), opus; returns its analysis as text and cannot write files. Dispatch via the Task tool (`subagent_type: planner`) before authoring.
- **Skill** `superpowers:writing-plans` — Stage 2 authoring methodology: turns the planner's analysis into a durable, bite-sized, TDD, file-by-file plan, saves it to the plan path, and drives the execution handoff. Load it before writing the plan document.

The command itself does not duplicate the planning method — it orchestrates context gathering, delegates analysis to the `planner` agent, then hands that analysis to `superpowers:writing-plans` to author and publish the plan. Never write code until the user confirms.
...
The `planner` agent source lives at `agents/planner.md` for manual installs. If the `planner` subagent is genuinely unavailable, perform the Stage 1 analysis inline; if `superpowers:writing-plans` is unavailable, author the plan inline using the PRD Artifact Output format above. Never surface a raw "Agent/skill not found" error — degrade gracefully and tell the user which stage ran inline.
